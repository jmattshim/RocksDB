...
// RAII wrapper for CSDVirt to ensure proper cleanup on thread termination
class CSDVirtWrapper {
 public:
  CSDVirtWrapper() : initialized_(false) {}

  ~CSDVirtWrapper() {
    if (initialized_) {
      csdvirt_.csdvirt_release_dev();
    }
  }
  ...
  CSDVirt* Get() {
    if (!initialized_) {
      ...
      csdvirt_.csdvirt_init_dev(csd_dev.c_str());
      initialized_ = true;
    }
    return &csdvirt_;
  }
...
// Thread-local CSDVirt helper
CSDVirt* GetThreadLocalCSDVirt() {
  thread_local CSDVirtWrapper wrapper;
  return wrapper.Get();
}
...
void Version::Get(const ReadOptions& read_options, const LookupKey& k,
...
#ifdef CONFIG_PROFILE_GET
  uint64_t get_stat[7] = {0, 0, 0, 0, 0, 0, 0};
  uint64_t stat_micros, start_micros;
#endif
  ...
  std::string file_name_lists[4];
  int search_level = 0;
  bool matched = false;
  
  int sample_rate = 40; // %
  bool offload_flag = read_options.read_offload && (rand() % 100 < sample_rate);
  ...
  while (f != nullptr) {
    ...
    // offload to CSD if enabled and SStable level > 1 
    // (most level 0/1 files are in cache)
    if (offload_flag && fp.GetHitFileLevel() > 1) {
#ifdef CONFIG_PROFILE_GET
      stat_micros = env_->NowMicros();
#endif
      auto& fd = f->file_metadata->fd;
      TableReader* t = fd.table_reader;

      auto skip_filters = IsFilterSkipped(
          static_cast<int>(fp.GetHitFileLevel()), fp.IsHitFileLastInLevel());

      file_name_lists[search_level] =
          TableFileName(cfd_->ioptions()->cf_paths, f->fd.GetNumber(), 0);
      if (t == nullptr) {
        // TableReader not in cache -> search from index block
        // printf("level: %d, file: %ld\n", fp.GetHitFileLevel(), f->fd.GetNumber());
        param.rocksdb_read_params.index_offset[search_level] =
            f->fd.file_size - f->file_metadata->tail_size;
        if (f->file_metadata->filter_size > 0) {
          param.rocksdb_read_params.index_offset[search_level] +=
              f->file_metadata->filter_size + 5;
        }
        param.rocksdb_read_params.total_size[search_level] =
            param.rocksdb_read_params.index_offset[search_level] +
            f->file_metadata->index_size - 5;
        param.rocksdb_read_params.num_data_blocks[search_level] =
            f->file_metadata->num_data_blocks;
        param.rocksdb_read_params.restart_offset[search_level] =
            param.rocksdb_read_params.total_size[search_level] -
            f->file_metadata->num_data_blocks * 4 - 5 + 1;
        param.rocksdb_read_params.search_type[search_level] =
            0;  // from index block
        search_level++;
      } else {
        // TableReader in cache -> search index in memory and check if data
        // block is cached
        BlockBasedTable* bbt = (BlockBasedTable*)t;
        bool is_included = false;

        param.rocksdb_read_params.search_type[search_level] = 0;  // initialize

        *status =
            bbt->CacheGet(read_options, ikey, &get_context,
                          mutable_cf_options_.prefix_extractor.get(),
                          skip_filters, &param, search_level, &is_included);

        if (get_context.State() == GetContext::kFound ||
            get_context.State() == GetContext::kDeleted) {
          search_level = 0; // no offload needed
          break;
        }

        if (is_included) {
          // search offload needed
          param.rocksdb_read_params.index_offset[search_level] =
              f->fd.file_size - f->file_metadata->tail_size;
          if (f->file_metadata->filter_size > 0) {
            param.rocksdb_read_params.index_offset[search_level] +=
...
#ifdef CONFIG_PROFILE_GET
      get_stat[0] += env_->NowMicros() - stat_micros;
#endif
    } else {
  ...
  if (offload_flag && search_level > 0) {
    // offload read to CSD
    bool found = false;
    int i;
#ifdef CONFIG_PROFILE_GET
    stat_micros = env_->NowMicros();
#endif
    CSDVirt* csdvirt = GetThreadLocalCSDVirt();
#ifdef CONFIG_PROFILE_GET
    get_stat[1] = env_->NowMicros() - stat_micros;
#endif

    csdvirt->csdvirt_set_debugging_info(0);

    if (vset_->db_options()->read_magic) {
      /* ===== Magic Read Path ===== */
      struct magic_params magic_param;
      memset(&magic_param, 0, sizeof(struct magic_params));

      // Copy read params
      magic_param.read.read_params.search_level = search_level;
      magic_param.read.read_params.key_length = user_key.size();
      memcpy(magic_param.read.read_params.key, user_key.data(), user_key.size());
      for (i = 0; i < search_level; i++) {
        magic_param.read.read_params.index_offset[i] = param.rocksdb_read_params.index_offset[i];
        magic_param.read.read_params.restart_offset[i] = param.rocksdb_read_params.restart_offset[i];
        magic_param.read.read_params.total_size[i] = param.rocksdb_read_params.total_size[i];
        magic_param.read.read_params.num_data_blocks[i] = param.rocksdb_read_params.num_data_blocks[i];
        magic_param.read.read_params.block_offset[i] = param.rocksdb_read_params.block_offset[i];
        magic_param.read.read_params.block_size[i] = param.rocksdb_read_params.block_size[i];
        magic_param.read.read_params.filter_offset[i] = param.rocksdb_read_params.filter_offset[i];
        magic_param.read.read_params.filter_size[i] = param.rocksdb_read_params.filter_size[i];
        magic_param.read.read_params.search_type[i] = param.rocksdb_read_params.search_type[i];
      }

      // Compute actual sizes per level
      size_t actual_sizes[4];
      for (i = 0; i < search_level; i++) {
        actual_sizes[i] = (param.rocksdb_read_params.search_type[i] == 0)
            ? param.rocksdb_read_params.total_size[i]
            : param.rocksdb_read_params.block_size[i];
      }

      // Single magic read call — CSD handles SLM alloc and file loading
      size_t result_size = 0;
#ifdef CONFIG_PROFILE_GET
      stat_micros = env_->NowMicros();
#endif
      csdvirt->csdvirt_magic_read(
          file_name_lists, search_level, actual_sizes,
          &magic_param, &result_size);
#ifdef CONFIG_PROFILE_GET
      get_stat[4] = env_->NowMicros() - stat_micros;
#endif

      size_t dev_output_buf = magic_param.info.output_buf;
      size_t magic_buf_size = magic_param.info.output_buf_size;

      // Read result from SLM
      char output_buf[16384];
#ifdef CONFIG_PROFILE_GET
      stat_micros = env_->NowMicros();
#endif
      csdvirt->csdvirt_read_slm(output_buf, dev_output_buf, magic_buf_size);
#ifdef CONFIG_PROFILE_GET
      get_stat[5] = env_->NowMicros() - stat_micros;
#endif

      // Parse result
      struct rocksdb_read_output* read_output =
          (struct rocksdb_read_output*)output_buf;
      if (read_output->key_length == 0 ||
          read_output->key_length != user_key.size()) {
        found = false;
        csdvirt->csdvirt_release_memory(dev_output_buf);
        printf("Key(%s) is not found in CSD magic read offload!!!!\n",
      ...
      if (found) {
        ...
        Slice slice(output_buf + sizeof(struct rocksdb_read_output) +
                      read_output->key_length,
                      read_output->value_length);
        parsed_key =
            ParsedInternalKey(user_key, read_output->sequence_number, kTypeValue);
        get_context.SaveValue(parsed_key, slice, &matched, status);
      }

      // Cleanup: only output buffer (CSD managed input internally)
#ifdef CONFIG_PROFILE_GET
      stat_micros = env_->NowMicros();
#endif
      csdvirt->csdvirt_release_memory(dev_output_buf);
#ifdef CONFIG_PROFILE_GET
      get_stat[2] += env_->NowMicros() - stat_micros;
#endif
    ...
    /* Alloc SLM range */
    size_t aligned_level_size[4];
    size_t total_size = 0;
    for (i = 0; i < search_level; i++) {
      if (param.rocksdb_read_params.search_type[i] == 0) {
        aligned_level_size[i] =
            ALIGNED_UP(param.rocksdb_read_params.total_size[i], 16 * 1024);
      } else {
        aligned_level_size[i] =
            ALIGNED_UP(param.rocksdb_read_params.block_size[i], 16 * 1024);
      }
      total_size += aligned_level_size[i];
    }
    size_t output_size = 16 * 1024;

#ifdef CONFIG_PROFILE_GET
    stat_micros = env_->NowMicros();
#endif
    size_t dev_input_buf = csdvirt->csdvirt_alloc_memory(total_size);
    size_t dev_output_buf = csdvirt->csdvirt_alloc_memory(output_size);
    size_t append_offset = 0;
    // printf("%s: %lu %lu\n", user_key.ToString().c_str(), dev_input_buf /
    // 16384, dev_output_buf / 16384);
#ifdef CONFIG_PROFILE_GET
    get_stat[2] = env_->NowMicros() - stat_micros;
#endif

    // csdvirt->csdvirt_set_debugging_info(g_debug_counter.fetch_add(1,
    ...
    for (i = 0; i < search_level; i++) {
      if (param.rocksdb_read_params.search_type[i] == 0) {
        csdvirt->csdvirt_load_files(&file_name_lists[i], 1,
                                    dev_input_buf + append_offset,
                                    &param.rocksdb_read_params.total_size[i]);
      } else {
        csdvirt->csdvirt_load_part_of_file(
            &file_name_lists[i], 1, dev_input_buf + append_offset,
            &param.rocksdb_read_params.total_size[i],
            param.rocksdb_read_params.block_offset[i],
            param.rocksdb_read_params.block_size[i]);
      }
      param.rocksdb_read_params.start_offset[i] = append_offset;
      append_offset += aligned_level_size[i];
    }
#ifdef CONFIG_PROFILE_GET
    get_stat[3] = env_->NowMicros() - stat_micros;
#endif

    /* Perform read */
    param.rocksdb_read_params.search_level = search_level;
    memcpy(param.rocksdb_read_params.key, user_key.data(), user_key.size());
    param.rocksdb_read_params.key_length = user_key.size();
    // printf("key: %s (%d)\n", param.rocksdb_read_params.key,
    // param.rocksdb_read_params.key_length);

    size_t result_size = 0;
#ifdef CONFIG_PROFILE_GET
    stat_micros = env_->NowMicros();
#endif
    csdvirt->csdvirt_execute(ROCKSDB_READ_PROGRAM_INDEX, dev_input_buf,
                             dev_output_buf, total_size, &param,
                             sizeof(struct CSD_PARAMS), &result_size);
#ifdef CONFIG_PROFILE_GET
    get_stat[4] = env_->NowMicros() - stat_micros;
#endif
    ...
    stat_micros = env_->NowMicros();
#endif
    csdvirt->csdvirt_read_slm(output_buf, dev_output_buf, result_size);
#ifdef CONFIG_PROFILE_GET
    get_stat[5] = env_->NowMicros() - stat_micros;
#endif
    ...
    if (read_output->key_length == 0 ||
      ...
      csdvirt->csdvirt_release_memory(dev_output_buf);
      printf("Key(%s) is not found in CSD read offload!!!!\n",
             user_key.ToString().c_str());
      printf("Key(%s) is not found in CSD read offload!!!!\n",
             user_key.ToString().c_str());
      printf("Key(%s) is not found in CSD read offload!!!!\n",
      ...
      // printf("value length: %d, x count: %d\n", read_output->value_length,
      // count);
      found = true;
    }

    if (found) {
      // same logic in BlockBasedTable::Get()
      ParsedInternalKey parsed_key;
      Slice slice(output_buf + sizeof(struct rocksdb_read_output) +
                    read_output->key_length,
                    read_output->value_length);
    ...
    // Note: csdvirt device is kept open as thread-local storage
    // It will be automatically closed when thread terminates
    }

#ifdef CONFIG_PROFILE_GET
    get_stat[6] = env_->NowMicros() - start_micros;
    ROCKS_LOG_INFO(
        info_log_, "[stat] CSD read: %lu %lu %lu %lu %lu %lu %lu, %d, %d\n",
        get_stat[0], get_stat[1], get_stat[2], get_stat[3], get_stat[4],
        get_stat[5], get_stat[6], search_level, fp.GetHitFileLevel());
#endif
  } else {
#ifdef CONFIG_PROFILE_GET
    get_stat[6] = env_->NowMicros() - start_micros;
    ROCKS_LOG_INFO(
        info_log_, "[stat] Host read: %lu %lu %lu %lu %lu %lu %lu, %d, %d\n",
        get_stat[0], get_stat[1], get_stat[2], get_stat[3], get_stat[4],
        get_stat[5], get_stat[6], search_level, fp.GetHitFileLevel());
#endif
  }
      ...
      for (int level = 0; level < cfd->NumberLevels(); level++) {
        ...
        for (const auto& f : level_files) {
          ...
          edit.AddFile(level, f->fd.GetNumber(), f->fd.GetPathId(),
                       ...
                       f->index_size, f->num_entries, f->filter_size,
                       f->num_data_blocks);
        }
      }
