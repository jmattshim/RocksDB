...
#undef CONFIG_VERIFY_COMPACTION

#ifdef CONFIG_VERIFY_COMPACTION
#include <fcntl.h>
#include <unistd.h>
#endif
...
void CompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {
  ...
  uint64_t prev_micros = db_options_.clock->NowMicros();
  ...
  Status s;
  uint64_t stat_micros;
  #ifdef CONFIG_PROFILE_COMPACTION
  uint64_t compaction_stat[6];
  memset(compaction_stat, 0, sizeof(uint64_t) * 6);
  /*
  * // For Host Compaction
  * stat[0]: iterator related (read)
  * stat[1]: merge (compute)
  * stat[2]: open file
  * stat[3]: add file
  * stat[4]: finish file
  *
  * // For CSD Compaction
  * stat[0]: get file info
  * stat[1]: SLMCPY
  * stat[2]: EXEC
  * stat[3]: NSCPY
  * stat[4]: read meta
  ...
  */
  ...
  int num_files_for_compaction = sub_compact->compaction->num_input_files(0) + sub_compact->compaction->num_input_files(1);

  if (db_options_.csd_offload && sub_compact->compaction->level() && num_files_for_compaction < 30 &&
      sub_compact->compaction->num_input_files(0) > 0 && sub_compact->compaction->num_input_files(1) > 0) {
    /* CSD Compaction */
    sub_compact->compaction_job_stats.is_csd_compaction = true;
    CSDVirt csdvirt;
    printf("[CSD%s] Compacting %lu@%d + %lu@%d files\n",
           db_options_.csd_magic ? "-Magic" : "",
           sub_compact->compaction->num_input_files(0), sub_compact->compaction->level(),
           sub_compact->compaction->num_input_files(1),
           sub_compact->compaction->level() + 1);

    std::string csd_dev = get_csd_device_path();
    csdvirt.csdvirt_init_dev(csd_dev.c_str());

    /* 1. Load SSTable to SLM */
    /* 1-1. Collect input table information */
    struct CSDCompaction csd_compaction;
    size_t total_file_size = 0;
    RecordStartTime(&stat_micros);
    for (int which = 0; which < 2; which++) {
      csd_compaction.nfiles[which] = sub_compact->compaction->num_input_files(which);
      csd_compaction.file_name_lists[which] = new std::string[csd_compaction.nfiles[which]];
      csd_compaction.file_size_lists[which] = (size_t*)malloc(sizeof(size_t) * csd_compaction.nfiles[which]);
      csd_compaction.level_size[which] = 0;
      for (int i = 0; i < csd_compaction.nfiles[which]; i++) {
        csd_compaction.file_name_lists[which][i] = GetTableFileName(sub_compact->compaction->input(which, i)->fd.GetNumber());
        // printf("lvl %d, fname: %s\n", which, csd_compaction.file_name_lists[which][i].c_str());
        csd_compaction.file_size_lists[which][i] = sub_compact->compaction->input(which, i)->fd.file_size - 
            sub_compact->compaction->input(which, i)->tail_size;
        csd_compaction.level_size[which] += csd_compaction.file_size_lists[which][i];
      }
      total_file_size += csd_compaction.level_size[which];
    }
#ifdef CONFIG_PROFILE_COMPACTION
    compaction_stat[0] += GetElapsedTime(stat_micros);
#endif

    uint64_t datablock_threshold = sub_compact->compaction->target_output_file_size() * 0.99;
    int max_output_files = (int)(total_file_size / datablock_threshold) + 1;
    csd_compaction.max_output_size = max_output_files * sub_compact->compaction->target_output_file_size();
    // printf("total file size is: %lu, max file size is: %lu\n", total_file_size, csd_compaction.max_output_size);
    if (sub_compact->compaction->target_output_file_size() > 64 * 1024 * 1024) {
      printf("large SST happend!!!!\n");
      exit(1);
    }

    /* Prepare common metadata needed for both magic and standard paths */
    csd_compaction.starting_file_number = versions_->FetchAddFileNumber(max_output_files);
    ...
    uint64_t oldest_ancester_time =
        sub_compact->compaction->MinInputFileOldestAncesterTime(nullptr, nullptr);
    ...
    // Get bits_per_key from the filter policy in BlockBasedTableOptions
    int bits_per_key = 0;
    bool bloom_flag = false;
    const auto* table_factory = sub_compact->compaction->immutable_options()->table_factory.get();
    if (table_factory) {
      const auto* bbto = table_factory->GetOptions<BlockBasedTableOptions>();
      if (bbto && bbto->filter_policy) {
        const auto* policy = bbto->filter_policy.get();
        if (policy->IsInstanceOf(BloomLikeFilterPolicy::kClassName())) {
          bits_per_key = static_cast<const BloomLikeFilterPolicy*>(policy)->GetWholeBitsPerKey();
          bloom_flag = true;
        }
      }
    }

    size_t dev_output_buf = 0;
    size_t result_size = 0;

    if (db_options_.csd_magic) {
      /* ============ Magic Command Path ============ */
      /* CSD handles memory allocation and file loading internally */
      RecordStartTime(&stat_micros);

      struct magic_params magic_param;
      memset(&magic_param, 0, sizeof(struct magic_params));
      magic_param.compaction.first_level_size = csd_compaction.level_size[0];
      magic_param.compaction.second_level_size = csd_compaction.level_size[1];
      magic_param.compaction.sstable_size = sub_compact->compaction->target_output_file_size();
      magic_param.compaction.datablock_threshold = datablock_threshold;
      magic_param.compaction.block_size = 4096;
      magic_param.compaction.crc_flag = db_options_.crc_accel;
      magic_param.compaction.crc_cores = db_options_.crc_accel ? 2 : 0;
      magic_param.compaction.bits_per_key = bits_per_key;
      magic_param.compaction.bloom_flag = bloom_flag;

      magic_param.compaction.host_properties.file_number = csd_compaction.starting_file_number;
      magic_param.compaction.host_properties.creation_time = 0;
      magic_param.compaction.host_properties.file_creation_time = current_time;
      memcpy(magic_param.compaction.host_properties.db_id, db_id_.c_str(), db_id_.size());
      magic_param.compaction.host_properties.db_id[db_id_.size()] = '\0';
      memcpy(magic_param.compaction.host_properties.db_session_id, db_session_id_.c_str(), db_session_id_.size());
      magic_param.compaction.host_properties.db_session_id[db_session_id_.size()] = '\0';

      csdvirt.csdvirt_magic_compaction(
          csd_compaction.file_name_lists[0], csd_compaction.nfiles[0], csd_compaction.file_size_lists[0],
          csd_compaction.file_name_lists[1], csd_compaction.nfiles[1], csd_compaction.file_size_lists[1],
          &magic_param, &result_size);

      dev_output_buf = magic_param.info.output_buf;
      size_t dev_metadata_buf = magic_param.info.output_second_buf;
      size_t magic_buf_size = magic_param.info.output_buf_size;
      if (magic_buf_size == 0) {
        printf("ERROR: magic_buf_size is 0, cannot perform wrap-around\n");
        exit(1);
      }
#ifdef CONFIG_PROFILE_COMPACTION
      compaction_stat[2] += GetElapsedTime(stat_micros);
#endif

      if (result_size == 0xFFFFFFFF) {
        result_size = csd_compaction.max_output_size;
      }

      /* Handle output - write files using fiemap */
      /* Magic command uses circular buffer with wrap-around */
      ...
      while ((remaining > (size_t) 0) && !cfd->IsDropped()) {
        {
          RecordStartTime(&stat_micros);
          FileMetaData meta;
          meta.fd = FileDescriptor(csd_compaction.starting_file_number + iter_num, sub_compact->compaction->output_path_id(), 0);
          meta.oldest_ancester_time = oldest_ancester_time;
          meta.file_creation_time = current_time;
          meta.epoch_number = epoch_number;
          meta.temperature = temperature;
          s = GetSstInternalUniqueId(db_id_, db_session_id_, meta.fd.GetNumber(), &meta.unique_id);
          if (!s.ok()) {
            ROCKS_LOG_ERROR(db_options_.info_log,
                            "[%s] [JOB %d] file #%" PRIu64 " failed to generate unique id: %s.",
                            cfd->GetName().c_str(), job_id_, meta.fd.GetNumber(), s.ToString().c_str());
            exit(1);
          }
          sub_compact->Current().AddOutput(std::move(meta), cfd->internal_comparator(), paranoid_file_checks_);
        }
#ifdef CONFIG_PROFILE_COMPACTION
        compaction_stat[5] += GetElapsedTime(stat_micros);
#endif

        std::string fname = GetTableFileName(csd_compaction.starting_file_number + iter_num);
        struct fiemap* fiemap = (struct fiemap*)csdvirt.csdvirt_make_and_get_file_extent(
                            fname, sub_compact->compaction->target_output_file_size());

        size_t write_size = (remaining < sub_compact->compaction->target_output_file_size() ?
                            remaining : sub_compact->compaction->target_output_file_size());
        size_t write_offset = 0;
        size_t output_size = 0;

        RecordStartTime(&stat_micros);
        while ((write_size > (size_t) 0) && !cfd->IsDropped()) {
          size_t request_size = MAX_IO_SPLIT_SIZE;
          if (write_size < request_size) {
            request_size = write_size;
          }

          /* Calculate wrapped offset within circular buffer */
          size_t wrapped_offset = total_data_offset % magic_buf_size;
          /* Limit request_size to not cross buffer boundary */
          if (wrapped_offset + request_size > magic_buf_size) {
            request_size = magic_buf_size - wrapped_offset;
          }

          size_t length = csdvirt.csdvirt_write_file(fiemap, dev_output_buf + wrapped_offset, write_offset, &request_size);
          ...
          if (length != request_size) {
            ...
            output_size += length;
            total_data_offset += length;
            finish_flag = true;
            break;
          }

          write_offset += request_size;
          total_data_offset += request_size;
          write_size -= request_size;
          output_size += request_size;
        }
#ifdef CONFIG_PROFILE_COMPACTION
        compaction_stat[3] += GetElapsedTime(stat_micros);
#endif
        remaining -= output_size;
        iter_num++;
        total_io_size += output_size;
      ...
      /* Read output SSTable metadata and apply */
      /* In magic mode, metadata is in separate buffer (dev_metadata_buf) */
      size_t* file_size = NULL;
      struct compaction_stats csd_stats;
      struct compacted_file_metadata* sst_meta = NULL;

      if (!cfd->IsDropped()) {
        RecordStartTime(&stat_micros);
        int num_outputs;
        csdvirt.csdvirt_read_slm(&num_outputs, dev_metadata_buf, sizeof(int));
        file_size = (size_t*)malloc(sizeof(size_t) * num_outputs);
        csdvirt.csdvirt_read_slm(file_size, dev_metadata_buf + sizeof(int), sizeof(size_t) * num_outputs);

        csdvirt.csdvirt_read_slm(&csd_stats, dev_metadata_buf + sizeof(int) + sizeof(size_t) * MAX_OUTPUT_TABLES, sizeof(struct compaction_stats));
        sub_compact->Current().SetBytesWritten(total_io_size);
        sub_compact->Current().SetNumOutputRecords(csd_stats.num_output_entries);
        sub_compact->Current().SetNumOutputFiles(num_outputs);
        sub_compact->compaction_job_stats.num_input_records = csd_stats.num_input_entries;
        sub_compact->compaction_job_stats.total_input_raw_key_bytes = csd_stats.raw_input_key_size;
        sub_compact->compaction_job_stats.total_input_raw_value_bytes = csd_stats.raw_input_value_size;

        sst_meta = (struct compacted_file_metadata *)malloc(sizeof(struct compacted_file_metadata) * num_outputs);
        csdvirt.csdvirt_read_slm((void*)sst_meta, dev_metadata_buf + sizeof(int) + sizeof(size_t) * MAX_OUTPUT_TABLES + sizeof(struct compaction_stats), sizeof(struct compacted_file_metadata) * num_outputs);
#ifdef CONFIG_PROFILE_COMPACTION
        compaction_stat[4] += GetElapsedTime(stat_micros);
#endif

        RecordStartTime(&stat_micros);
        for (int i = 0; i < num_outputs; i++) {
          FileMetaData* meta = sub_compact->Current().GetSpecificMetaData(i);
          if (meta->fd.GetNumber() != csd_compaction.starting_file_number + i) {
            printf("ERROR on Output SSTable management\n");
            exit(1);
          }

          std::string fname = GetTableFileName(csd_compaction.starting_file_number + i);
          csdvirt.csdvirt_truncate_file(fname, file_size[i]);

          Slice smallest_key(sst_meta[i].smallest_key, sst_meta[i].smallest_klen);
          Slice largest_key(sst_meta[i].largest_key, sst_meta[i].largest_klen);
          meta->SetBoundaries(smallest_key, largest_key, sst_meta[i].smallest_seqno, sst_meta[i].largest_seqno);

          meta->fd.file_size = file_size[i];
          meta->tail_size = sst_meta[i].tail_size;
          meta->index_size = sst_meta[i].properties.index_size;
          meta->filter_size = sst_meta[i].properties.filter_size;
          meta->num_entries = sst_meta[i].properties.num_entries;
          meta->num_data_blocks = sst_meta[i].properties.num_data_blocks;

          sub_compact->Current().SetSpecificFinished(i, true);
          ...
          if (meta->smallest.size() > 0 && meta->largest.size() > 0) {
            ...
            if (!new_largest.empty() && !new_smallest.empty()) {
              refined_oldest_ancester_time =
                  sub_compact->compaction->MinInputFileOldestAncesterTime(&(meta->smallest), &(meta->largest));
              if (refined_oldest_ancester_time != std::numeric_limits<uint64_t>::max()) {
          ...
          TableProperties tp;
          tp.column_family_id = cfd->GetID();
          tp.column_family_name = cfd->GetName();
          tp.file_creation_time = current_time + i;
          tp.orig_file_number = csd_compaction.starting_file_number + i;
          tp.db_id = db_id_;
          tp.db_session_id = db_session_id_;
          tp.db_host_id = cfd->ioptions()->db_host_id;
          if (!ReifyDbHostIdProperty(cfd->ioptions()->env, &tp.db_host_id).ok()) {
            ROCKS_LOG_INFO(cfd->ioptions()->logger, "db_host_id property will not be set");
          }
          tp.index_key_is_user_key = 1;
          tp.compression_name = CompressionTypeToString(sub_compact->compaction->output_compression());
          CompressionOptions compress_opt = sub_compact->compaction->output_compression_opts();
          tp.compression_options = CompressionOptionsToString(compress_opt);
          tp.comparator_name = cfd->ioptions()->user_comparator != nullptr
                                   ? cfd->ioptions()->user_comparator->Name() : "nullptr";
          tp.merge_operator_name = "nullptr";
          tp.prefix_extractor_name = "nullptr";
          tp.property_collectors_names = "[]";

          tp.data_size = sst_meta[i].properties.data_size;
          tp.index_size = sst_meta[i].properties.index_size;
          tp.filter_size = sst_meta[i].properties.filter_size;
          tp.raw_key_size = sst_meta[i].properties.raw_key_size;
          tp.raw_value_size = sst_meta[i].properties.raw_value_size;
          tp.num_entries = sst_meta[i].properties.num_entries;
          tp.num_data_blocks = sst_meta[i].properties.num_data_blocks;
          tp.tail_start_offset = sst_meta[i].properties.tail_start_offset;

          sub_compact->Current().SetSpecificTableProperties(i, tp);

          ROCKS_LOG_INFO(db_options_.info_log,
                     "[%s] [JOB %d] Generated table #%" PRIu64 ": %" PRIu64
                     " keys, %" PRIu64 " bytes%s, temperature: %s",
                     cfd->GetName().c_str(), job_id_, tp.orig_file_number,
                     tp.num_entries, meta->fd.file_size,
                     meta->marked_for_compaction ? " (need compaction)" : "",
                     temperature_to_string[meta->temperature].c_str());

          EventHelpers::LogAndNotifyTableFileCreationFinished(
              event_logger_, cfd->ioptions()->listeners, dbname_, cfd->GetName(), fname,
              job_id_, meta->fd, meta->oldest_blob_file_number, tp,
              TableFileCreationReason::kCompaction, s, kUnknownFileChecksum,
              kUnknownFileChecksumFuncName);

          auto sfm = static_cast<SstFileManagerImpl*>(db_options_.sst_file_manager.get());
          if (sfm && meta != nullptr && meta->fd.GetPathId() == 0) {
            ...
            if (sfm->IsMaxAllowedSpaceReached()) {
              ...
              TEST_SYNC_POINT("CompactionJob::FinishCompactionOutputFile:MaxAllowedSpaceReached");
              InstrumentedMutexLock l(db_mutex_);
...
#ifdef CONFIG_VERIFY_COMPACTION
        /* Verify CSD-reported key boundaries against actual SST file contents. */
        {
          bool has_mismatch = false;
          bool has_overlap = false;

          /* First pass: check all files and dump full debug info on any error */
          for (int i = 0; i < num_outputs && s.ok(); i++) {
            std::string vfname = GetTableFileName(csd_compaction.starting_file_number + i);
            /* Flush page cache so reads hit the device after NVMe copy */
            int vfd = open(vfname.c_str(), O_RDONLY);
            if (vfd >= 0) { posix_fadvise(vfd, 0, 0, POSIX_FADV_DONTNEED); close(vfd); }
            Options verify_options;
            verify_options.env = db_options_.env;
            EnvOptions env_options(verify_options);
            SstFileDumper dumper(verify_options, vfname, Temperature::kUnknown,
                                 2 * 1024 * 1024, false, false, false,
                                 env_options, true /* silent */);
            if (!dumper.getStatus().ok()) {
              fprintf(stderr, "[CSD KEY VERIFY] Failed to open SST %s: %s\n",
                      vfname.c_str(), dumper.getStatus().ToString().c_str());
              fflush(stderr);
              abort();
            }

            TableReader* reader = dumper.GetTableReader();
            ReadOptions ro;
            InternalIterator* viter = reader->NewIterator(
                ro, nullptr, nullptr, false, TableReaderCaller::kCompaction);

            Slice csd_smallest(sst_meta[i].smallest_key, sst_meta[i].smallest_klen);
            Slice csd_largest(sst_meta[i].largest_key, sst_meta[i].largest_klen);

            bool smallest_ok = true, largest_ok = true;
            viter->SeekToFirst();
            if (viter->Valid() && viter->key().compare(csd_smallest) != 0) smallest_ok = false;
            viter->SeekToLast();
            if (viter->Valid() && viter->key().compare(csd_largest) != 0) largest_ok = false;

            if (!smallest_ok || !largest_ok) has_mismatch = true;

            /* Check consecutive overlap */
            if (i > 0) {
              InternalKey prev_largest_ik, cur_smallest_ik;
              prev_largest_ik.DecodeFrom(Slice(sst_meta[i - 1].largest_key, sst_meta[i - 1].largest_klen));
              cur_smallest_ik.DecodeFrom(csd_smallest);
              if (cfd->internal_comparator().Compare(prev_largest_ik, cur_smallest_ik) >= 0) {
                has_overlap = true;
              }
            }
          ...
          if (has_mismatch || has_overlap) {
            fprintf(stderr, "\n========== [CSD KEY VERIFY] COMPACTION DEBUG DUMP ==========\n");
            fprintf(stderr, "Compaction: %lu@L%d + %lu@L%d -> %d output files\n",
                    (unsigned long)sub_compact->compaction->num_input_files(0),
                    sub_compact->compaction->level(),
                    (unsigned long)sub_compact->compaction->num_input_files(1),
                    sub_compact->compaction->level() + 1,
                    num_outputs);
            fprintf(stderr, "Starting file number: %" PRIu64 "\n\n",
                    (uint64_t)csd_compaction.starting_file_number);

            /* Dump input files */
            for (int which = 0; which < 2; which++) {
              fprintf(stderr, "--- Input level %d (%d files) ---\n",
                      sub_compact->compaction->level() + which,
                      (int)sub_compact->compaction->num_input_files(which));
              for (size_t f = 0; f < sub_compact->compaction->num_input_files(which); f++) {
                const FileMetaData* fmeta = sub_compact->compaction->input(which, f);
                fprintf(stderr, "  file #%" PRIu64 " size=%" PRIu64
                                " smallest=%s largest=%s\n",
                        fmeta->fd.GetNumber(), fmeta->fd.file_size,
                        fmeta->smallest.DebugString(true).c_str(),
                        fmeta->largest.DebugString(true).c_str());
              }
            }

            /* Dump ALL output files: CSD metadata vs actual SST */
            fprintf(stderr, "\n--- Output files (%d) ---\n", num_outputs);
            for (int i = 0; i < num_outputs; i++) {
              uint64_t fnum = csd_compaction.starting_file_number + i;
              std::string vfname = GetTableFileName(fnum);

              ParsedInternalKey csd_small_ik, csd_large_ik;
              Slice csd_s(sst_meta[i].smallest_key, sst_meta[i].smallest_klen);
              Slice csd_l(sst_meta[i].largest_key, sst_meta[i].largest_klen);
              ParseInternalKey(csd_s, &csd_small_ik, true);
              ParseInternalKey(csd_l, &csd_large_ik, true);

              fprintf(stderr, "\n  [%d] file #%" PRIu64 " (%s)\n"
                              "    file_size=%" PRIu64 " num_entries=%" PRIu64
                              " num_data_blocks=%" PRIu64 " tail_size=%" PRIu64 "\n"
                              "    smallest_klen=%u largest_klen=%u"
                              " smallest_seqno=%" PRIu64 " largest_seqno=%" PRIu64 "\n"
                              "    CSD smallest: %s\n"
                              "    CSD largest:  %s\n",
                      i, fnum, vfname.c_str(),
                      (uint64_t)file_size[i],
                      sst_meta[i].properties.num_entries,
                      sst_meta[i].properties.num_data_blocks,
                      (uint64_t)sst_meta[i].tail_size,
                      sst_meta[i].smallest_klen, sst_meta[i].largest_klen,
                      (uint64_t)sst_meta[i].smallest_seqno,
                      (uint64_t)sst_meta[i].largest_seqno,
                      csd_small_ik.DebugString(true, true).c_str(),
                      csd_large_ik.DebugString(true, true).c_str());

              /* Now open SST and read actual first/last key */
              int vfd2 = open(vfname.c_str(), O_RDONLY);
              if (vfd2 >= 0) { posix_fadvise(vfd2, 0, 0, POSIX_FADV_DONTNEED); close(vfd2); }
              Options verify_options;
              verify_options.env = db_options_.env;
              EnvOptions env_options(verify_options);
              SstFileDumper dumper(verify_options, vfname, Temperature::kUnknown,
              ...
              if (dumper.getStatus().ok()) {
                ...
                viter->SeekToFirst();
                if (viter->Valid()) {
                  ParsedInternalKey actual_ik;
                  ParseInternalKey(viter->key(), &actual_ik, true);
                  bool match = (viter->key().compare(csd_s) == 0);
                  fprintf(stderr, "    SST smallest: %s %s\n",
                          actual_ik.DebugString(true, true).c_str(),
                          match ? "(OK)" : "*** MISMATCH ***");
                }
                ...
                if (viter->Valid()) {
                  ParsedInternalKey actual_ik;
                  ParseInternalKey(viter->key(), &actual_ik, true);
                  bool match = (viter->key().compare(csd_l) == 0);
                  fprintf(stderr, "    SST largest:  %s %s\n",
                          actual_ik.DebugString(true, true).c_str(),
                          match ? "(OK)" : "*** MISMATCH ***");
                }
                ...
                fprintf(stderr, "    SST: FAILED TO OPEN: %s\n",
                        dumper.getStatus().ToString().c_str());
              }

              /* Dump raw struct bytes for smallest_key/largest_key */
              fprintf(stderr, "    raw smallest_key[32]: ");
              for (int b = 0; b < 32; b++) fprintf(stderr, "%02X", (unsigned char)sst_meta[i].smallest_key[b]);
              fprintf(stderr, "\n    raw largest_key[32]:  ");
              for (int b = 0; b < 32; b++) fprintf(stderr, "%02X", (unsigned char)sst_meta[i].largest_key[b]);
              fprintf(stderr, "\n");
            }
        ...
        /* Verify output key range does not exceed input key range */
        if (num_outputs > 0) {
          const Comparator* ucmp = cfd->user_comparator();
          Slice input_smallest_ukey, input_largest_ukey;
          bool first_input = true;
          for (int which = 0; which < 2; which++) {
            for (size_t f = 0; f < sub_compact->compaction->num_input_files(which); f++) {
              const FileMetaData* fmeta = sub_compact->compaction->input(which, f);
              Slice s_ukey = fmeta->smallest.user_key();
              Slice l_ukey = fmeta->largest.user_key();
              if (first_input || ucmp->Compare(s_ukey, input_smallest_ukey) < 0) {
                input_smallest_ukey = s_ukey;
              }
              if (first_input || ucmp->Compare(l_ukey, input_largest_ukey) > 0) {
                input_largest_ukey = l_ukey;
              }
              first_input = false;
            }
          }

          Slice out_first_ik(sst_meta[0].smallest_key, sst_meta[0].smallest_klen);
          Slice out_last_ik(sst_meta[num_outputs - 1].largest_key, sst_meta[num_outputs - 1].largest_klen);
          Slice output_smallest_ukey = ExtractUserKey(out_first_ik);
          Slice output_largest_ukey = ExtractUserKey(out_last_ik);

          bool range_exceeded = false;
          if (ucmp->Compare(output_smallest_ukey, input_smallest_ukey) < 0) {
            range_exceeded = true;
          }
          if (ucmp->Compare(output_largest_ukey, input_largest_ukey) > 0) {
            range_exceeded = true;
          }

          if (range_exceeded) {
            fprintf(stderr, "\n========== [CSD-Magic RANGE CHECK] OUTPUT EXCEEDS INPUT RANGE ==========\n");
            fprintf(stderr, "Compaction: %lu@L%d + %lu@L%d -> %d output files\n",
                    (unsigned long)sub_compact->compaction->num_input_files(0),
                    sub_compact->compaction->level(),
                    (unsigned long)sub_compact->compaction->num_input_files(1),
                    sub_compact->compaction->level() + 1,
                    num_outputs);
            fprintf(stderr, "Input  user key range: [%s] .. [%s]\n",
                    input_smallest_ukey.ToString(true).c_str(),
                    input_largest_ukey.ToString(true).c_str());
            fprintf(stderr, "Output user key range: [%s] .. [%s]\n",
                    output_smallest_ukey.ToString(true).c_str(),
                    output_largest_ukey.ToString(true).c_str());

            for (int which = 0; which < 2; which++) {
              fprintf(stderr, "\n--- Input level %d (%d files) ---\n",
                      sub_compact->compaction->level() + which,
                      (int)sub_compact->compaction->num_input_files(which));
              for (size_t f = 0; f < sub_compact->compaction->num_input_files(which); f++) {
                const FileMetaData* fmeta = sub_compact->compaction->input(which, f);
            ...
            fprintf(stderr, "\n--- Output files (%d) ---\n", num_outputs);
            for (int i = 0; i < num_outputs; i++) {
              ParsedInternalKey pik_s, pik_l;
              Slice s_slice(sst_meta[i].smallest_key, sst_meta[i].smallest_klen);
              Slice l_slice(sst_meta[i].largest_key, sst_meta[i].largest_klen);
              ParseInternalKey(s_slice, &pik_s, true);
              ParseInternalKey(l_slice, &pik_l, true);
              fprintf(stderr, "  [%d] file #%" PRIu64 " smallest=%s largest=%s\n",
                      i, (uint64_t)(csd_compaction.starting_file_number + i),
                      pik_s.DebugString(true, true).c_str(),
                      pik_l.DebugString(true, true).c_str());
            }
...
#endif /* CONFIG_VERIFY_COMPACTION */
      ...
      /* Clean up for magic command path */
      for (int which = 0; which < 2; which++) {
        delete [] csd_compaction.file_name_lists[which];
        free(csd_compaction.file_size_lists[which]);
      }
      if (file_size != NULL) free(file_size);
      if (sst_meta != NULL) free(sst_meta);
      csdvirt.csdvirt_release_memory(dev_metadata_buf);
      csdvirt.csdvirt_release_memory(dev_output_buf);
      csdvirt.csdvirt_release_dev();

      sub_compact->status = s;
    } else {
    /* ============ Standard CSD Path (manual memory allocation) ============ */

    /* 1-2. Load SSTable to SLM */
    RecordStartTime(&stat_micros);
    size_t aligned_first_level_size = ALIGNED_UP(csd_compaction.level_size[0], 16 * 1024);
    size_t aligned_second_level_size = ALIGNED_UP(csd_compaction.level_size[1], 16 * 1024);
    size_t dev_input_buf = csdvirt.csdvirt_alloc_memory(aligned_first_level_size + aligned_second_level_size);
    if (dev_input_buf == (size_t)-1) {
      fprintf(stderr, "[CSD] SLM alloc failed for input buf (size=%lu)\n",
              aligned_first_level_size + aligned_second_level_size);
      abort();
    }
    dev_output_buf = csdvirt.csdvirt_alloc_memory(csd_compaction.max_output_size + FRONT_METADATA_SIZE);
    if (dev_output_buf == (size_t)-1) {
      fprintf(stderr, "[CSD] SLM alloc failed for output buf (size=%lu)\n",
              csd_compaction.max_output_size + FRONT_METADATA_SIZE);
      csdvirt.csdvirt_release_memory(dev_input_buf);
      abort();
    }
    size_t dev_intermediate_buf = 0; // can be used for crc acceleration
    size_t dev_second_intermediate_buf = 0; // can be used for crc acceleration

    csdvirt.csdvirt_set_debugging_info(0);
    csdvirt.csdvirt_load_files(csd_compaction.file_name_lists[0], csd_compaction.nfiles[0],
            dev_input_buf, csd_compaction.file_size_lists[0]);
    csdvirt.csdvirt_set_debugging_info(1);
    csdvirt.csdvirt_load_files(csd_compaction.file_name_lists[1], csd_compaction.nfiles[1],
            dev_input_buf + aligned_first_level_size, csd_compaction.file_size_lists[1]);
#ifdef CONFIG_PROFILE_COMPACTION
    compaction_stat[1] += GetElapsedTime(stat_micros);
#endif

    /* 2. Perform Compaction */
    /* 2-1. Fill compaction related metadata */
    struct CSD_PARAMS param;
    memset(&param, 0, sizeof(struct CSD_PARAMS));
    param.rocksdb_compaction_params.first_level_size = csd_compaction.level_size[0];
    param.rocksdb_compaction_params.second_level_start = aligned_first_level_size;
    param.rocksdb_compaction_params.second_level_size = csd_compaction.level_size[1];
    param.rocksdb_compaction_params.sstable_size = sub_compact->compaction->target_output_file_size();
    param.rocksdb_compaction_params.datablock_threshold = datablock_threshold;
    param.rocksdb_compaction_params.block_size = 4096;
    param.rocksdb_compaction_params.crc_flag = true;
    param.rocksdb_compaction_params.bloom_flag = bloom_flag;
    param.rocksdb_compaction_params.bits_per_key = bits_per_key;

    param.rocksdb_compaction_params.host_properties.file_number = csd_compaction.starting_file_number;
    param.rocksdb_compaction_params.host_properties.creation_time = 0; // doesn't use oldest_ancester_time. why?
    param.rocksdb_compaction_params.host_properties.file_creation_time = current_time;
    memcpy(param.rocksdb_compaction_params.host_properties.db_id, db_id_.c_str(), db_id_.size());
    param.rocksdb_compaction_params.host_properties.db_id[db_id_.size()] = '\0';
    memcpy(param.rocksdb_compaction_params.host_properties.db_session_id, db_session_id_.c_str(), db_session_id_.size());
    param.rocksdb_compaction_params.host_properties.db_session_id[db_session_id_.size()] = '\0';

    /* 2-2. Send execute command */
    RecordStartTime(&stat_micros);
    size_t remaining = 0;
    if (!db_options_.crc_accel) {
      csdvirt.csdvirt_execute(ROCKSDB_COMPACTION_PROGRAM_INDEX, dev_input_buf, dev_output_buf, csd_compaction.max_output_size + FRONT_METADATA_SIZE, &param, sizeof(struct CSD_PARAMS), &result_size);
    } else {
      struct CSD_PARAMS crc_param, crc_two_param;
      memset(&crc_param, 0, sizeof(struct CSD_PARAMS));
      crc_param.rocksdb_crc_params.sstable_size = sub_compact->compaction->target_output_file_size();
      crc_param.rocksdb_crc_params.datablock_threshold = datablock_threshold;
      memset(&crc_two_param, 0, sizeof(struct CSD_PARAMS));
      crc_two_param.rocksdb_crc_params.sstable_size = sub_compact->compaction->target_output_file_size();
      crc_two_param.rocksdb_crc_params.datablock_threshold = datablock_threshold;

      dev_intermediate_buf = csdvirt.csdvirt_alloc_memory(csd_compaction.max_output_size + FRONT_METADATA_SIZE);
      if (dev_intermediate_buf == (size_t)-1) {
        fprintf(stderr, "[CSD] SLM alloc failed for intermediate buf (size=%lu)\n",
                csd_compaction.max_output_size + FRONT_METADATA_SIZE);
        csdvirt.csdvirt_release_memory(dev_input_buf);
        csdvirt.csdvirt_release_memory(dev_output_buf);
        abort();
      }
      dev_second_intermediate_buf = csdvirt.csdvirt_alloc_memory(csd_compaction.max_output_size + FRONT_METADATA_SIZE);
      if (dev_second_intermediate_buf == (size_t)-1) {
        fprintf(stderr, "[CSD] SLM alloc failed for second intermediate buf (size=%lu)\n",
                csd_compaction.max_output_size + FRONT_METADATA_SIZE);
        csdvirt.csdvirt_release_memory(dev_input_buf);
        csdvirt.csdvirt_release_memory(dev_output_buf);
        csdvirt.csdvirt_release_memory(dev_intermediate_buf);
        abort();
      }
      
      param.rocksdb_compaction_params.crc_flag = false;
      csdvirt.csdvirt_execute(ROCKSDB_COMPACTION_PROGRAM_INDEX, dev_input_buf, dev_intermediate_buf, csd_compaction.max_output_size + FRONT_METADATA_SIZE, &param, sizeof(struct CSD_PARAMS), &result_size);
      crc_param.rocksdb_crc_params.num_cores = 2;
      crc_param.rocksdb_crc_params.core_id = 0;
      csdvirt.csdvirt_execute(ROCKSDB_CRC_PROGRAM_INDEX, dev_intermediate_buf, dev_second_intermediate_buf, csd_compaction.max_output_size + FRONT_METADATA_SIZE, &crc_param, sizeof(struct CSD_PARAMS), &result_size);
      crc_two_param.rocksdb_crc_params.num_cores = 2;
      crc_two_param.rocksdb_crc_params.core_id = 1;
      csdvirt.csdvirt_execute(ROCKSDB_CRC_PROGRAM_INDEX, dev_second_intermediate_buf, dev_output_buf, csd_compaction.max_output_size + FRONT_METADATA_SIZE, &crc_two_param, sizeof(struct CSD_PARAMS), &result_size);
    }
#ifdef CONFIG_PROFILE_COMPACTION
    compaction_stat[2] += GetElapsedTime(stat_micros);
#endif
    ...
    while ((remaining > (size_t) 0) && !cfd->IsDropped()) {
      // OpenCompactionOutputFile()
      {
        ...
        sub_compact->Current().AddOutput(std::move(meta), cfd->internal_comparator(),
      ...
      size_t write_size = (remaining < sub_compact->compaction->target_output_file_size() ?
                          remaining : sub_compact->compaction->target_output_file_size());
      size_t write_offset = 0;
      size_t dev_temp_buf = (dev_output_buf + FRONT_METADATA_SIZE) +
                            (iter_num * sub_compact->compaction->target_output_file_size());
      size_t output_size = 0;
      ...
      while ((write_size > (size_t) 0) && !cfd->IsDropped()) {
        ...
        size_t length = csdvirt.csdvirt_write_file(fiemap, dev_temp_buf, write_offset, &request_size);
        if (length > 0 && length != request_size) {
          if (length == (size_t) -1) {
            printf("IOCTL failed\n");
            exit(1);
          }
          printf("exit pattern: %lu\n", length);
          output_size += length;
          finish_flag = true;
          break;
        }
    ...
    // printf("total IO is: %lu\n", total_io_size);

    /* 4. Read output SSTable metadata and apply */
    size_t* file_size = NULL;
    struct compaction_stats csd_stats;
    struct compacted_file_metadata* sst_meta = NULL;
    // perform finalization only when output is successfully handled
    if (!cfd->IsDropped()) {
      RecordStartTime(&stat_micros);
      /* 4-1. Read output SSTable metadata from CSD */
      int num_outputs;
      csdvirt.csdvirt_read_slm(&num_outputs, dev_output_buf, sizeof(int));
      // printf("num_outputs : %d\n", num_outputs);
      file_size = (size_t*)malloc(sizeof(size_t) * num_outputs);
      csdvirt.csdvirt_read_slm(file_size, dev_output_buf + sizeof(int), sizeof(size_t) * num_outputs);

      csdvirt.csdvirt_read_slm(&csd_stats, dev_output_buf + sizeof(int) + sizeof(size_t) * MAX_OUTPUT_TABLES, sizeof(struct compaction_stats));
      /* Fill stats */
      sub_compact->Current().SetBytesWritten(total_io_size);
      sub_compact->Current().SetNumOutputRecords(csd_stats.num_output_entries);
      sub_compact->Current().SetNumOutputFiles(num_outputs);
      ...
      sst_meta = (struct compacted_file_metadata *)malloc(sizeof(struct compacted_file_metadata) * num_outputs);
      csdvirt.csdvirt_read_slm((void*)sst_meta, dev_output_buf + sizeof(int) + sizeof(size_t) * MAX_OUTPUT_TABLES + sizeof(struct compaction_stats), sizeof(struct compacted_file_metadata) * num_outputs);
#ifdef CONFIG_PROFILE_COMPACTION
      compaction_stat[4] += GetElapsedTime(stat_micros);
#endif 
      // for (int i = 0; i < num_outputs; i++) {
      //   printf("%d - %lu, %s, %s, %lu, %lu\n", i, sst_meta[i].tail_size, sst_meta[i].smallest_key, sst_meta[i].largest_key, sst_meta[i].smallest_seqno, sst_meta[i].largest_seqno);
      // }

      /* 4-2. Apply metadata to RocksDB */
      RecordStartTime(&stat_micros);
      for (int i = 0; i < num_outputs; i++) {
        FileMetaData* meta = sub_compact->Current().GetSpecificMetaData(i);
        if (meta->fd.GetNumber() != csd_compaction.starting_file_number + i) {
        ...
        /* Make TableProperties */
        TableProperties tp;
        tp.column_family_id = cfd->GetID();
        tp.column_family_name = cfd->GetName();
        tp.file_creation_time = current_time + i; // just use first SST's creation time + 1
        tp.orig_file_number = csd_compaction.starting_file_number + i;
        tp.db_id = db_id_;
        tp.db_session_id = db_session_id_;
        tp.db_host_id = cfd->ioptions()->db_host_id;
        ...
        tp.compression_options = CompressionOptionsToString(compress_opt);
        tp.comparator_name = cfd->ioptions()->user_comparator != nullptr
                                 ? cfd->ioptions()->user_comparator->Name()
                                 : "nullptr";
        tp.merge_operator_name = "nullptr";
        tp.prefix_extractor_name = "nullptr";
        std::string property_collectors_names = "[]";
        tp.property_collectors_names = property_collectors_names;
      ...
      if (num_outputs > 0) {
        ...
        if (range_exceeded) {
          fprintf(stderr, "\n========== [CSD RANGE CHECK] OUTPUT EXCEEDS INPUT RANGE ==========\n");
          fprintf(stderr, "Compaction: %lu@L%d + %lu@L%d -> %d output files\n",
                  (unsigned long)sub_compact->compaction->num_input_files(0),
                  sub_compact->compaction->level(),
                  (unsigned long)sub_compact->compaction->num_input_files(1),
    ...
    if (db_options_.crc_accel) {
      csdvirt.csdvirt_release_memory(dev_intermediate_buf);
      csdvirt.csdvirt_release_memory(dev_second_intermediate_buf);
    }
    csdvirt.csdvirt_release_dev();

    sub_compact->status = s;
    } /* end of standard CSD path else block */
  } /* end of CSD compaction block */
  else {
    /* Host Compaction */
    printf("[Host] Compacting %lu@%d + %lu@%d files\n",
         sub_compact->compaction->num_input_files(0), sub_compact->compaction->level(),
         sub_compact->compaction->num_input_files(1),
         sub_compact->compaction->level() + 1);
    ...
    last_cpu_micros = prev_cpu_micros;
  ...
  if (db_options_.csd_offload && sub_compact->compaction->level() && num_files_for_compaction < 30 &&
      sub_compact->compaction->num_input_files(0) > 0 && sub_compact->compaction->num_input_files(1) > 0) {
    ROCKS_LOG_INFO(db_options_.info_log,
                    "[stat] [%lu@%d + %lu@%d] csd compaction: %lu", 
                    sub_compact->compaction->num_input_files(0), sub_compact->compaction->level(),
                    sub_compact->compaction->num_input_files(1), sub_compact->compaction->level() + 1,
                    (db_options_.clock->NowMicros() - prev_micros));
#ifdef CONFIG_PROFILE_COMPACTION
    ROCKS_LOG_INFO(db_options_.info_log,
                    "[CSD-breakdown] [%lu@%d + %lu@%d] csd compaction: %lu %lu %lu %lu %lu %lu %lu", 
                    sub_compact->compaction->num_input_files(0), sub_compact->compaction->level(),
                    sub_compact->compaction->num_input_files(1), sub_compact->compaction->level() + 1,
                    (db_options_.clock->NowMicros() - prev_micros),
                    compaction_stat[0], compaction_stat[1], compaction_stat[2], 
                    compaction_stat[3], compaction_stat[4], compaction_stat[5]);
#endif
  } else {
    ROCKS_LOG_INFO(db_options_.info_log,
                    "[stat] [%lu@%d + %lu@%d] host compaction: %lu",
                    sub_compact->compaction->num_input_files(0), sub_compact->compaction->level(),
                    sub_compact->compaction->num_input_files(1), sub_compact->compaction->level() + 1,
                    (db_options_.clock->NowMicros() - prev_micros));
#ifdef CONFIG_PROFILE_COMPACTION
    if (sub_compact->compaction->level()) {
      ROCKS_LOG_INFO(db_options_.info_log,
                      "[Host-breakdown] [%lu@%d + %lu@%d] host compaction: %lu",
                      sub_compact->compaction->num_input_files(0), sub_compact->compaction->level(),
                      sub_compact->compaction->num_input_files(1), sub_compact->compaction->level() + 1,
                      (db_options_.clock->NowMicros() - prev_micros));
    }
...
bool CompactionJob::UpdateCompactionStats(uint64_t* num_input_range_del) {
  ...
  for (int input_level = 0;
    ...
    for (size_t i = 0; i < num_input_files; ++i) {
      ...
      if (! compaction_job_stats_->is_csd_compaction) {
