...
class CompactionJob {
 public:
  ...
  void RecordStartTime(uint64_t *stat_micros) {
#ifdef CONFIG_PROFILE_COMPACTION
    *stat_micros = db_options_.env->NowMicros();
#endif
  }

  uint64_t GetElapsedTime(uint64_t stat_micros) {
#ifdef CONFIG_PROFILE_COMPACTION
    return (db_options_.env->NowMicros() - stat_micros);
#endif
    return 0;
  }
...
struct CSDCompaction {
  int nfiles[2];
  std::string* file_name_lists[2];
  size_t* file_size_lists[2];
  size_t level_size[2];
  size_t max_output_size;
  uint64_t starting_file_number;
};
