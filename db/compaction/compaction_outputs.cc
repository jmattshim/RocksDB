...
Status CompactionOutputs::Finish(
  ...
  if (s.ok()) {
    ...
    meta->index_size = builder_->GetTableProperties().index_size;
    meta->num_entries = builder_->GetTableProperties().num_entries;
    meta->filter_size = builder_->GetTableProperties().filter_size;
    meta->num_data_blocks = builder_->GetTableProperties().num_data_blocks;
  }
...
bool CompactionOutputs::ShouldStopBefore(const CompactionIterator& c_iter) {
  ...
  // if (current_output_file_size_ >= (compaction_->max_output_file_size() * 0.99 - 4137)) {
