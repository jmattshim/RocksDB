...
class CompactionOutputs {
 public:
  ...
  void SetBytesWritten(uint64_t bytes) { stats_.bytes_written = bytes; }
  ...
  FileMetaData* GetSpecificMetaData(size_t i) { return &specific_output(i).meta; }
  void SetSpecificTableProperties(size_t i, TableProperties tp) {
    specific_output(i).table_properties = std::make_shared<TableProperties>(tp);
  }
  void SetSpecificFinished(size_t i, bool finished) {
    specific_output(i).finished = finished;
  }
 ...
 private:
  ...
  Output& specific_output(size_t i) {
    assert(!outputs_.empty() || i < outputs_.size());
