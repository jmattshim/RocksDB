...
Status ImportColumnFamilyJob::Run() {
  ...
  for (size_t i = 0; s.ok() && i < files_to_import_.size(); ++i) {
    ...
    for (size_t j = 0; s.ok() && j < files_to_import_[i].size(); ++j) {
      ...
      index_size = f.table_properties.index_size;
      uint64_t num_entries = f.table_properties.num_entries;
      uint64_t filter_size = f.table_properties.filter_size;
      uint64_t num_data_blocks = f.table_properties.num_data_blocks;
      ...
      dummy_version_edit.AddFile(
          ...
          tail_size,
          static_cast<bool>(
              f.table_properties.user_defined_timestamps_persisted), index_size,
          num_entries, filter_size, num_data_blocks);
