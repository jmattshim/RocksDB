 ...
 private:
  ...
  Status ScanTable(TableInfo* t) {
    ...
    if (status.ok()) {
      ...
      t->meta.index_size = props->index_size;
      t->meta.num_entries = props->num_entries;
      t->meta.filter_size = props->filter_size;
      t->meta.num_data_blocks = props->num_data_blocks;
    }
    ...
    for (const auto& cf_id_and_tables : cf_id_to_tables) {
      ...
      for (const auto* table : cf_id_and_tables.second) {
        ...
        dummy_edit.AddFile(
            ...
            table->meta.user_defined_timestamps_persisted, table->meta.index_size,
            table->meta.num_entries, table->meta.filter_size,
            table->meta.num_data_blocks);
      }
