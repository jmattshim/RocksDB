...
Status FileMetaData::SetBoundaries(const Slice& smallest_key, const Slice& largest_key,
                        SequenceNumber smallest_seqno, SequenceNumber largest_seqno) {                      
  smallest.DecodeFrom(smallest_key);
  largest.DecodeFrom(largest_key);

  fd.smallest_seqno = smallest_seqno;
  fd.largest_seqno = largest_seqno;
...
bool VersionEdit::EncodeTo(std::string* dst,
  ...
  for (size_t i = 0; i < new_files_.size(); i++) {
    ...
    if (f.index_size) {
      PutVarint32(dst, NewFileCustomTag::kIndexSize);
      std::string varint_index_size;
      PutVarint64(&varint_index_size, f.index_size);
      PutLengthPrefixedSlice(dst, Slice(varint_index_size));
    }
    if (f.num_entries) {
      PutVarint32(dst, NewFileCustomTag::kNumEntries);
      std::string varint_num_entries;
      PutVarint64(&varint_num_entries, f.num_entries);
      PutLengthPrefixedSlice(dst, Slice(varint_num_entries));
    }
    if (f.filter_size) {
    ...
    if (f.num_data_blocks) {
      PutVarint32(dst, NewFileCustomTag::kNumDataBlocks);
      std::string varint_num_data_blocks;
      PutVarint64(&varint_num_data_blocks, f.num_data_blocks);
      PutLengthPrefixedSlice(dst, Slice(varint_num_data_blocks));
    }
    ...
    while (true) {
      ...
      switch (custom_tag) {
        ...
        case kTailSize:
          ...
          break;
        case kIndexSize:
          if (!GetVarint64(&field, &f.index_size)) {
            return "invalid index size";
          }
          break;
        case kNumEntries:
          if (!GetVarint64(&field, &f.num_entries)) {
            return "invalid num entries";
          }
          break;
        case kFilterSize:
          ...
          break;
        case kNumDataBlocks:
          if (!GetVarint64(&field, &f.num_data_blocks)) {
            return "invalid num data blocks";
          }
          break;
