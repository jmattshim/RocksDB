...
Status BlockBasedTable::CacheGet(const ReadOptions& original_read_options, const Slice& key,
                            GetContext* get_context,
                            const SliceTransform* prefix_extractor,
                            bool skip_filters, struct CSD_PARAMS* param,
                            int search_level, bool* is_included) {
  ReadOptions read_options = original_read_options;
  ...
  assert(get_context != nullptr);
  Status s;
  const bool no_io = true; // ensure no io for cache get
  ...
  // filter exists but key definitely not in table
  if (!may_match) {
    *is_included = false;
    return Status::OK();
  }
  ...
  if (iiter->status().IsIncomplete()) {
    // couldn't get index block from block_cache
    // CSD should search from the index block
    param->rocksdb_read_params.search_type[search_level] = 0;
    *is_included = true;
    return Status::OK();
  }
  ...
  for (iiter->Seek(key); iiter->Valid() && !done; iiter->Next()) {
    ...
    // At this point, we have the metadata about the data block's 
    // offset and size.
    param->rocksdb_read_params.block_offset[search_level] = v.handle.offset();
    param->rocksdb_read_params.block_size[search_level] = v.handle.size();
    param->rocksdb_read_params.search_type[search_level] = 1;

    // Now check if data block is in cache
    BlockCacheLookupContext lookup_data_block_context{
        TableReaderCaller::kUserGet, tracing_get_id,
        /*get_from_user_specified_snapshot=*/read_options.snapshot !=
            nullptr};
    ...
    // Data block not in cache. Return to offload to CSD
    if (no_io && biter.status().IsIncomplete()) {
      *is_included = true;
      s = biter.status();
      return Status::OK();
