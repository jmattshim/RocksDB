...
void PropertyBlockBuilder::AddTableProperty(const TableProperties& props) {
  ...
  if (!props.db_id.empty()) {
    ...
    if (props.db_id.length() != 36) {
      printf("Different db_id length than expected!!\n");
    }
  }
  if (!props.db_session_id.empty()) {
    Add(TablePropertiesNames::kDbSessionId, props.db_session_id);
    if (props.db_session_id.length() != 20 ) {
      printf("Different db_session_id length than expected!!\n");
    }
  }
