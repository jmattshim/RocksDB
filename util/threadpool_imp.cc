...
// Set the thread name to aid debugging
    ...
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    CPU_SET(base_thread_core, &cpuset);
    pthread_setaffinity_np(th_handle, sizeof(cpu_set_t), &cpuset);
    printf("Thread %s bound to core %d\n", 
          thread_name_stream.str().c_str(), base_thread_core);
    base_thread_core += 4;
